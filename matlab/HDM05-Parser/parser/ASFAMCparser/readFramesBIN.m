function [result,mot] = readFramesBIN(skel,mot,fid,range)
...
while (~feof(fid))
    ...
    h = fscanf(fid,'%d',1); % read nDOF 
    fseek(fid,2,'cof'); % skip carriage return and line feed bytes
	...
	if (isempty(boneIndex))
        fclose(fid);
        error(['AMC: Unknown bone "' token '" in file: ' mot.filename '.BIN!']);
	end
...
data = fread(fid,[totalNumDOF inf],'double');
fclose(fid);
