...
% SA: we need a separate conversion factor for skeleton nodes, since
% mot.angleUnit may not equal skel.angleUnit
switch lower(skel.angleUnit)
    case 'deg'
        conversion_factor_skel = pi/180;
    case 'rad'
        conversion_factor_skel = 1;
    otherwise
        error(['Unknown angle unit: ' skel.angleUnit]);
end
...
for k = 1:nTraj
    ...
    if nRotDOF > 0
        ...
        axis_quat = repmat(euler2quat(flipud(node.axis)*conversion_factor_skel,'zyx'),1,mot.nframes); % According to ASF specs, rotation order for "axis" should be XYZ. However, they use the opposite multiplication order as we do!
