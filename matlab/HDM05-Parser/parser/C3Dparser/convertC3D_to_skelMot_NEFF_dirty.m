function [skel, mot] = convertC3D_to_skelMot_NEFF_dirty(Markers, ParameterGroup, VideoFrameRate, c3dFilename)
...
labels = cell(0,0);
for i = 1:length(labelsIdx)
    newLabels = ParameterGroup(pointIdx).Parameter(labelsIdx(i)).data;
...
for i = 1:size(Markers,2)
    ...
    %mot.nameMap{i,1} = sprintf('Marker%04d', i);
    mot.nameMap{i,2} = 0;
    mot.nameMap{i,3} = i;
end    
