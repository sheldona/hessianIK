...
%       fid=fopen(FullFileName,'r','d'); % DEC VAX D floating point and VAX ordering
    fid=fopen(FullFileName,'r','ieee-le'); % DEC VAX D floating point and VAX ordering
end
...
%AB: hack to enable 32 bit integer for number of frames: read 32bit instead
%of 16 bit and then shift 2 bytes to the beginning of the file.
EndFrame=fread(fid,1,'int32');			        %# of last video frame
fseek(fid, -2, 'cof');
...
h = waitbar(0,[escapeUnderscore(FileName),' is loading...']);
...
%     speedUp = false;
    ...
    ResidualError = int8(zeros(NvideoFrames, Nmarkers));
    CameraInfo = int8(zeros(NvideoFrames, Nmarkers));
    for i=1:NvideoFrames
        fprintf('%5d', i);
        for j=1:Nmarkers
...
%             CameraInfo(i,j)=fread(fid,1,'int8');
            Markers(i,j,1:3)=freadVAXD(fid,3,'int16')'.*Scale;
            ResidualError(i,j)=freadVAXD(fid,1,'int8');
            CameraInfo(i,j)=freadVAXD(fid,1,'int8');
        end
        waitbar(i/NvideoFrames)
        for j=1:NanalogFramesPerVideoFrame,
%             AnalogSignals(j+NanalogFramesPerVideoFrame*(i-1),1:NanalogChannels)=...
%                 fread(fid,NanalogChannels,'int16')';
           AnalogSignals(j+NanalogFramesPerVideoFrame*(i-1),1:NanalogChannels)=...
                freadVAXD(fid,NanalogChannels,'int16')';
